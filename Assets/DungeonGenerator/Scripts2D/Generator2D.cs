...
public class Generator2D : MonoBehaviour
{
    ...
    char GetCellChar(CellType cellType)
    {
        ...
        {
            case CellType.Room:
                return '+'; // Represent room cell
            case CellType.Hallway:
                return 'O'; // Represent hallway cell
            default:
                return ' '; // Represent empty cell and any other undefined types
        }
    }
    ...
    {
        if (delaunay == null || delaunay.Triangles == null) return;

        Gizmos.color = Color.yellow; // Set gizmo color to yellow for visibility
        ...
        foreach (var triangle in delaunay.Triangles)
        {
            ...
            Gizmos.DrawLine(new Vector3(triangle.A.Position.x, 0, triangle.A.Position.y),
                            new Vector3(triangle.B.Position.x, 0, triangle.B.Position.y));

            // Draw line from vertex B to vertex C
            Gizmos.DrawLine(new Vector3(triangle.B.Position.x, 0, triangle.B.Position.y),
                            new Vector3(triangle.C.Position.x, 0, triangle.C.Position.y));

            // Draw line from vertex C to vertex A
            Gizmos.DrawLine(new Vector3(triangle.C.Position.x, 0, triangle.C.Position.y),
                            new Vector3(triangle.A.Position.x, 0, triangle.A.Position.y));
        }
    }
}
